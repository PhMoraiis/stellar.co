...
const Footer = () => {
	const renderFooterNavs = () => {
		const navItems = [
			{ title: "Sobre", route: "https://stellarstudio.com#about" },
			{ title: "Soluções", route: "https://stellarstudio.com#solutions" },
				...
				{navItems.map((item) => (
					...
					>
						{item.route.startsWith("http") ? (
							...
							<SmoothScrollLink href={item.route} offset={-100} duration={1.8}>
								<CenterUnderline label={item.title} />
							</SmoothScrollLink>
						)}
					</li>
		...
		<footer className="py-16 px-4 md:py-16 pb-6 lg:pt-20 bg-[#171718] font-movatif">
			<div className="px-4 lg:px-20 sm:px-6 py-12 md:py-10">
				...
				<div className="text-gray-300 md:text-xl flex items-start lg:items-center flex-col mt-6 lg:mt-12 md:flex-row md:justify-between lg:flex-row lg:justify-between text-xl lg:text-2xl space-y-6">
					<p>© 2026 Stellar Studio. Todos os direitos reservados.</p>
